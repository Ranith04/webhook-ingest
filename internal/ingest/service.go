...
// Shutdown waits for all background goroutines to complete, or until the
...
func (s *Service) Shutdown(ctx context.Context) {
	...
	select {
	case <-done:
		s.log.Info("all background jobs completed successfully")
	case <-ctx.Done():
		s.log.Error("shutdown timeout exceeded, abandoning background jobs", "err", ctx.Err())
	}
}
