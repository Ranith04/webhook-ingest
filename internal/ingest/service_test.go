...
func TestConcurrentDuplicateDelivery(t *testing.T) {
	...
	if got.CallCount != 1 {
		t.Fatalf("stats call count %d, want 1", got.CallCount)
	}
}

func TestRecordingProcessingAfterResponse(t *testing.T) {
	srv, st := testutil.NewServer(t)
	eventID, callID, accountID := testutil.IDs(t, st)
	ctx := context.Background()
	...
	// Wait for background job to definitely finish
	...
	if !processed {
		t.Fatal("expected recording_processed to be true after background job finishes")
	}
}
